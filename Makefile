...
BUILD_TAGS?=bdc
BUILD_FLAGS = -ldflags "-X github.com/bdc/bdc/version.GitCommit=`git rev-parse --short=8 HEAD`"
...
build:
	CGO_ENABLED=0 go build $(BUILD_FLAGS) -tags '$(BUILD_TAGS)' -o build/bdc ./cmd/bdc/

build_race:
	CGO_ENABLED=0 go build -race $(BUILD_FLAGS) -tags '$(BUILD_TAGS)' -o build/bdc ./cmd/bdc

install:
	CGO_ENABLED=0 go install $(BUILD_FLAGS) -tags '$(BUILD_TAGS)' ./cmd/bdc
...
draw_deps:
	...
	@goviz -i github.com/bdc/bdc/cmd/bdc -d 3 | dot -Tpng -o dependency-graph.png

get_deps_bin_size:
	@# Copy of build recipe with additional flags to perform binary size analysis
	$(eval $(shell go build -work -a $(BUILD_FLAGS) -tags '$(BUILD_TAGS)' -o build/bdc ./cmd/bdc/ 2>&1))
...
test_apps:
	...
	# requires `abci-cli` and `bdc` binaries installed
