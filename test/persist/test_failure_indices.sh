...
export TMHOME=$HOME/.bdc_persist
...
TM_CMD="bdc node --log_level=debug --rpc.laddr=unix://$RPC_ADDR" # &> bdc_${name}.log"
...
function start_procs(){
    name=$1
    indexToFail=$2
    echo "Starting persistent kvstore and bdc"
    ...
    # before starting bdc, remove the rpc socket
    rm -f $RPC_ADDR
    if [[ "$indexToFail" == "" ]]; then
		...
		if [[ "$CIRCLECI" == true ]]; then
            ...
            FAIL_TEST_INDEX=$indexToFail $TM_CMD &> "bdc_${name}.log"
		fi
        PID_bdc=$!
    fi
}
...
for failIndex in $(seq $failsStart $failsEnd); do
    ...
    # bdc should already have exited when it hits the fail index
    ...
    while [ "$ERR" != 0 ]; do
        ...
        i=$((i + 1))
        if [[ $i == 20 ]]; then
            echo "Timed out waiting for bdc to start"
            exit 1
        fi
    done
