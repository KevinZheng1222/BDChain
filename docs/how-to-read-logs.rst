...
Then bdc Core and the application perform a handshake.
...
::
    ...
    I[10-04|13:54:27.368] Completed ABCI Handshake - bdc and App are synced module=consensus appHeight=90 appHash=E0FBAFBF6FCED8B9786DDFEB1A0D4FA2501BADAD
...
::
    ...
    I[10-04|13:54:30.387] Starting WAL                                 module=consensus wal=/home/vagrant/.bdc/data/cs.wal/wal impl=WAL
    I[10-04|13:54:30.388] Starting TimeoutTicker                       module=consensus impl=TimeoutTicker

Notice the second row where bdc Core reports that "This node is a
...
Here is the list of modules you may encounter in bdc's log and a little
overview what they do.

- ``abci-client`` As mentioned in `Application Development Guide
  <app-development.html#abci-design>`__,
  bdc acts as an ABCI client with respect to the application and
  maintains 3 connections: mempool, consensus and query. The code used by
  bdc Core can be found `here
  <https://github.com/bdc/abci/tree/master/client>`__.
...
- ``consensus``
  The heart of bdc core, which is the implementation of the consensus
...
- ``events``
  ...
  `here
  <https://github.com/bdc/bdc/blob/master/types/events.go>`__.
...
- ``p2p``
  ...
  <https://github.com/bdc/bdc/blob/56c60fba2381e4ac41d2ae38a1eb6569acfbc095/p2p/README.md>`__.

- ``rpc``
  `bdc's RPC <specification/rpc.html>`__.

- ``rpc-server``
  RPC server. For implementation details, please read the `README <https://github.com/bdc/bdc/blob/master/rpc/lib/README.md>`__.
