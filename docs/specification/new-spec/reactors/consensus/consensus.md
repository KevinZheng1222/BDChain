# bdc Consensus Reactor

bdc Consensus is a distributed protocol executed by validator processes to agree on
the next block to be added to the bdc blockchain. The protocol proceeds in rounds, where
...
For efficiency reasons, validators in bdc consensus protocol do not agree directly on the
...
Validators in bdc communicate by peer-to-peer gossiping protocol. Each validator is connected
...
We now describe the content of each message exchanged during bdc consensus protocol.
...
NOTE: In the current version of the bdc, the consensus value in proposal is represented with
...
It is used in the gossip part of the bdc protocol to inform peers about a current
