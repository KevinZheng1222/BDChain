...
A bdc P2P network has different kinds of nodes with different requirements for connectivity to one another.
This document describes what kind of nodes bdc should enable and how they should work.
...
- a list of seeds, which can be provided to bdc via config file or flags,
...
The values `H` and `HASH` must be received and corroborated by means external to bdc, and specific to the user - ie. via the user's trusted social consensus.
...
dials those peers, and runs the bdc protocols with those it successfully connects to.

When the peer catches up to height H, it ensures the block hash matches HASH.
If not, bdc will exit, and the user must try again - either they are connected
