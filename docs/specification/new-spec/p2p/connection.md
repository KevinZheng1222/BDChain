...
The `msgPacket` is serialized using [go-wire](https://github.com/bdc/go-wire) and prefixed with 0x3.
...
using the `bdc/wire` submodule's `WriteBinary()` reflection routine.
