...
This document explains how bdc Peers are identified and how they connect to one another.

For details on peer discovery, see the [peer exchange (PEX) reactor doc](pex.md).

## Peer Identity

bdc peers are expected to maintain long-term persistent identities in the form of a public key.
...
In this case, the peer must be authenticated out-of-band of bdc,
...
two handhsakes are performed: one for authenticated encryption, and one for bdc versioning.
Both handshakes have configurable timeouts (they should complete quickly).

### Authenticated Encryption Handshake

bdc implements the Station-to-Station protocol
...
### bdc Version Handshake

The bdc Version Handshake allows the peers to exchange their NodeInfo:
