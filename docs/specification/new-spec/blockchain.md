...
Here we describe the data structures in the bdc blockchain and the rules for validating them.

## Data Structures

The bdc blockchains consists of a short list of basic data types:
...
bdc allows for multiple signature schemes to be used by prepending a single type-byte
to the signature bytes. Different signatures may also come with fixed or variable lengths.
Currently, bdc supports Ed25519 and Secp256k1.
