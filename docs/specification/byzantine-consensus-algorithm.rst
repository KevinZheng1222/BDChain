...
-  To *prevote* or *precommit* something means to broadcast a `prevote
   vote <https://godoc.org/github.com/bdc/bdc/types#Vote>`__
   or `first precommit
   vote <https://godoc.org/github.com/bdc/bdc/types#FirstPrecommit>`__
   for something.
...
-  Nodes gossip to nodes lagging in blockchain height with block
   `commits <https://godoc.org/github.com/bdc/bdc/types#Commit>`__
   for older blocks.
...
voting power. (see
`implementation <https://github.com/bdc/bdc/blob/develop/types/validator_set.go>`__)
...
limitation of bdc, but rather a limitation of all consensus
...
external to the weakly-synchronous bdc consensus algorithm. For
