...
-  blockchain protocol
   ...
   -  bdc Core maintains three connections:
...
file <https://github.com/bdc/abci/blob/master/types/types.proto>`__,
and the `protobuf
...
server in that language. bdc supports two kinds of implementation
of the server:

-  Asynchronous, raw socket server (bdc Socket Protocol, also
...
`Go <https://github.com/bdc/abci/tree/master/server>`__,
`JavaScript <https://github.com/bdc/js-abci>`__,
`Python <https://github.com/bdc/abci/tree/master/example/python3/abci>`__,
`C++ <https://github.com/mdyring/cpp-tmsp>`__, and
`Java <https://github.com/jbdc/jabci>`__.
...
ABCI server using the bdc Socket Protocol, known affectionately
...
command line. The other is a consensus engine, such as bdc Core,
which makes requests to the application every time a new transaction is
received or a block is committed.

It is unlikely that you will need to implement a client. For details of
our client, see
`here <https://github.com/bdc/abci/tree/master/client>`__.

Most of the examples below are from `kvstore application
<https://github.com/bdc/abci/blob/master/example/kvstore/kvstore.go>`__,
which is a part of the abci repo. `persistent_kvstore application
<https://github.com/bdc/abci/blob/master/example/kvstore/persistent_kvstore.go>`__
...
made between bdc Core and the application: the mempool connection
...
state to the latest committed state. bdc Core will then filter
...
DeliverTx is the workhorse of the blockchain. bdc sends the
...
Once all processing of the block is complete, bdc sends the
...
every block. It also allows bdc to send the current block hash
...
has run a successful Commit) so that it can tell bdc where to
...
one, include it in the list with a ``power`` equal to ``0``. bdc core
...
<https://godoc.org/github.com/bdc/bdc/lite#hdr-How_We_Track_Validators>`__
...
consensus. It's exposed over the bdc core rpc, so clients can
...
bdc Core currently uses the Query connection to filter peers upon
connecting, according to IP address or public key. For instance,
returning non-OK ABCI response to either of the following queries will
cause bdc to not connect to the corresponding peer:
...
When the app or bdc restarts, they need to sync to a common
height. When an ABCI connection is first established, bdc will
...
Using this information, bdc will determine what needs to be
replayed, if anything, against the app, to ensure both bdc and
the app are synced to the latest block height.

If the app returns a LastBlockHeight of 0, bdc will just replay
all blocks.
