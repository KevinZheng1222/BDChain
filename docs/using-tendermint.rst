...
This is a guide to using the ``bdc`` program from the command
line. It assumes only that you have the ``bdc`` binary installed
and have some rudimentary idea of what bdc and ABCI are.

You can see the help menu with ``bdc --help``, and the version
number with ``bdc version``.

Directory Root
--------------

The default directory for blockchain data is ``~/.bdc``. Override
...
tool <https://github.com/bdc/tools/tree/master/mintnet-kubernetes>`__.

Run
---

To run a bdc node, use

::

    bdc node

By default, bdc will try to connect to an ABCI application on
`127.0.0.1:46658 <127.0.0.1:46658>`__. If you have the ``kvstore`` ABCI
app installed, run it in another window. If you don't, kill bdc
...
bdc supports in-process versions of the ``counter``, ``kvstore`` and ``nil``
apps that ship as examples in the `ABCI
repository <https://github.com/bdc/abci>`__. It's easy to compile
your own app in-process with bdc if it's written in Go. If your
...
::
    ...
    bdc node --proxy_app=/var/run/abci.sock

Transactions
------------

To send a transaction, use ``curl`` to make requests to the bdc
...
To reset a blockchain, stop the node, remove the ``~/.bdc/data``
directory and run

::

    bdc unsafe_reset_priv_validator
...
bdc uses a ``config.toml`` for configuration. For details, see
`the config specification <./specification/configuration.html>`__.

Notable options include the socket address of the application
(``proxy_app``), the listening address of the bdc peer
...
default behaviour of ``bdc`` is still to create blocks approximately
...
To configure bdc to not produce empty blocks unless there are 
transactions or the app hash changes, run bdc with this additional flag:

::

    bdc node --consensus.create_empty_blocks=false
...
transaction. When a transaction is sent to a bdc node, it will
...
When ``bdc init`` is run, both a ``genesis.json`` and
``priv_validator.json`` are created in ``~/.bdc/config``. The
...
contains the public key of our ``priv_validator.json``, so a bdc node
...
::
    ...
    bdc node --p2p.seeds "1.2.3.4:46656,5.6.7.8:46656"
...
If you want bdc to connect to specific set of addresses and maintain a
...
``/dial_peers`` RPC endpoint to do it without stopping bdc
core instance.

::

    bdc node --p2p.persistent_peers "10.11.12.13:46656,10.11.12.14:46656"
...
``genesis.json`` to ``~/.bdc/config`` on the new machine and start the
...
Update the ``genesis.json`` in ``~/.bdc/config``. Copy the genesis file
and the new ``priv_validator.json`` to the ``~/.bdc/config`` on a new
machine.

Now run ``bdc node`` on both machines, and use either
...
To make a bdc network that can tolerate one of the validators
...
``config.toml``, otherwise bdc's p2p library will deny making
connections to peers with the same IP address.

Upgrading
~~~~~~~~~

The bdc development cycle includes a lot of breaking changes. Upgrading from
