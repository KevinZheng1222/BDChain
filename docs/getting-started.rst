...
As a general purpose blockchain engine, bdc is agnostic to the
application you want to run. So, to run a complete blockchain that does
something useful, you must start two programs: one is bdc Core,
the other is your application, which can be written in any programming
language. Recall from `the intro to ABCI <introduction.html#ABCI-Overview>`__ that
bdc Core handles all the p2p and consensus stuff, and just
...
``$GOPATH/bin`` in your
``$PATH``; see `here <https://github.com/bdc/bdc/wiki/Setting-GOPATH>`__ for more info.

Then run

::

    go get -u github.com/bdc/abci/cmd/abci-cli
...
::
    ...
    cd $GOPATH/src/github.com/bdc/abci
    make get_tools
...
In another terminal, we can start bdc. If you have never run
bdc before, use:
...
If you have used bdc, you may want to reset the data for a new
blockchain by running ``bdc unsafe_reset_all``. Then you can run
``bdc node`` to start bdc, and connect to the app. For
more details, see `the guide on using
bdc <./using-bdc.html>`__.

You should see bdc making blocks! We can get the status of our
bdc node as follows:
...
tuned for a future release that `makes this output more human-readable <https://github.com/bdc/abci/issues/32>`__.
...
Let's kill the previous instance of ``bdc`` and the ``kvstore``
...
In another window, reset then start bdc:

::

    bdc unsafe_reset_all
    bdc node
...
You'll also need to fetch the relevant repository, from `here <https://github.com/bdc/js-abci>`__ then install it. As go devs, we
keep all our code under the ``$GOPATH``, so run:

::

    go get github.com/bdc/js-abci &> /dev/null
    cd $GOPATH/src/github.com/bdc/js-abci/example
    npm install
    cd ..

Kill the previous ``counter`` and ``bdc`` processes. Now run the
app:
...
In another window, reset and start ``bdc``:
...
In this tutorial you learned how to run applications using bdc
...
But the true power of bdc comes from its ability to securely and
...
protocol. Next, we show you how to deploy bdc testnets.
