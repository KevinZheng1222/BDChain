...
Current logging system in bdc is very static and not flexible enough.

Issues: [358](https://github.com/bdc/bdc/issues/358), [375](https://github.com/bdc/bdc/issues/375).
...
First, we will need an interface for all of our libraries (`tmlibs`, bdc, etc.). My personal preference is go-kit `Logger` interface (see Appendix A.), but that is too much a bigger change. Plus we will still need levels.
...
This interface should go into `tmlibs/log`. All libraries which are part of the core (bdc/bdc) should obey it.

### 2) Logger with our current formatting

On top of this interface, we will need to implement a stdout logger, which will be used when bdc is configured to output logs to STDOUT.
...
Dynamic logger, which could be turned off for some modules at runtime. Public interface for other projects using bdc libraries.
