...
import (
	...
	//auto "github.com/bdc/tmlibs/autofile"
...
func (h *Handshaker) Handshake(proxyApp proxy.AppConns) error {
	...
	h.logger.Info("Completed ABCI Handshake - bdc and App are synced", "appHeight", blockHeight, "appHash", fmt.Sprintf("%X", appHash))
...
func (h *Handshaker) ReplayBlocks(state sm.State, appHash []byte, appBlockHeight int64, proxyApp proxy.AppConns) ([]byte, error) {
	...
	if storeBlockHeight == 0 {
		...
		// the state should never be ahead of the store (this is under bdc's control)
		cmn.PanicSanity(cmn.Fmt("StateBlockHeight (%d) > StoreBlockHeight (%d)", stateBlockHeight, storeBlockHeight))

	} else if storeBlockHeight > stateBlockHeight+1 {
		// store should be at most one ahead of the state (this is under bdc's control)
	...
	if storeBlockHeight == stateBlockHeight {
		// bdc ran Commit and saved the state.
...
func checkAppHash(state sm.State, appHash []byte) error {
	if !bytes.Equal(state.AppHash, appHash) {
		panic(fmt.Errorf("bdc state.AppHash does not match AppHash after replay. Got %X, expected %X", appHash, state.AppHash).Error())
	}
	return nil
}
