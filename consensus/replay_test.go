...
func testHandshakeReplay(t *testing.T, nBlocks int, mode uint) {
	...
	// run the chain through state.ApplyBlock to build up the bdc state
...
func buildAppStateFromChain(proxyApp proxy.AppConns, stateDB dbm.DB,
	...
	// TODO: get the genesis bytes (https://github.com/bdc/bdc/issues/1224)
...
func buildTMStateFromChain(config *cfg.Config, stateDB dbm.DB, state sm.State, chain []*types.Block, mode uint) sm.State {
	// run the whole chain against this client to build up the bdc state
