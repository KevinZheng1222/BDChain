...
import (
	...
	"github.com/bdc/bdc/lite/client"
	certerr "github.com/bdc/bdc/lite/errors"
...
func GetWithProofOptions(path string, key []byte, opts rpcclient.ABCIQueryOptions,
	...
	*ctypes.ResultABCIQuery, KeyProof, error) {
	...
	/* // TODO refactor so iavl stuff is not in bdc core
	   // https://github.com/bdc/bdc/issues/1183
	if len(resp.Value) > 0 {
...
func GetCertifiedCommit(h int64, node rpcclient.Client, cert lite.Certifier) (lite.Commit, error) {
	...
	// Validators and will fail on querying bdc for non-current height.
